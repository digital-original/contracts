 ...
 * @title ArtTokenBase
 *
 * @notice Upgradeable, abstract ERC-721 implementation used as a building block
 *         for the protocol's ArtToken contracts.
 * @notice Relies on OpenZeppelin upgradeable libraries and bundles the
 *         Enumerable and URIStorage extensions in a single inheritance tree.
 */
abstract contract ArtTokenBase is ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    /**
     * @dev Overrides {ERC721.approve}. Adds the `_beforeApprove` hook so that
     *      inheriting contracts can introduce custom approval rules.
     */
    ...
    /**
     * @dev Overrides {ERC721.setApprovalForAll}. Adds the
     *      `_beforeSetApprovalForAll` hook so that inheriting contracts can
     *      introduce custom operator-approval rules.
     */
    ...
    /**
     * @dev Helper that performs a safe mint and assigns a token URI in a single
     *      call.
     *
    ...
    /**
     * @dev Overrides the OpenZeppelin internal `_update` function in order to
     *      plug the `_beforeTransfer` hook.
     *
     * @param to      Address receiving the token.
     * @param tokenId Identifier of the token being transferred.
     * @param auth    Address whose approval is being used for the transfer (may
     *                be the the owner of the token or an operator).
     *
     ...
     * @param approved Boolean indicating whether the operator is being approved
     *                 or revoked.
     */
