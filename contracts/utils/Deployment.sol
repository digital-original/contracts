 ...
 *      1. {deployUpgradeableContract} — deploys a `TransparentUpgradeableProxy` pointing to an
...
library Deployment {
    /**
     * @notice Deploys a `TransparentUpgradeableProxy` with the provided implementation and admin.
        ...
        if (nonce == 0x00) {
            return
                address(
                    uint160(
                        uint256(
                            keccak256(abi.encodePacked(bytes1(0xd6), bytes1(0x94), account, bytes1(0x80)))
                        )
                    )
                );
        } else if (nonce <= 0x7f) {
            return
                address(
                    uint160(
                        uint256(
                            keccak256(
                                abi.encodePacked(bytes1(0xd6), bytes1(0x94), account, bytes1(uint8(nonce)))
                            )
                        )
                    )
                );
