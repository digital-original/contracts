 ...
 *
 * @notice Lightweight helper for recovering the signer of an EIP-712 typed-data
 *         signature. Combines OpenZeppelin's {ECDSA} and {MessageHashUtils}.
 *
 * @dev Reverts with {EIP712SignatureZeroAddress} if signature recovery yields
 *      the zero address (which indicates an invalid signature).
 */
library EIP712Signature {
    /**
     * @notice Recovers the signer of an EIP-712 `{structHash}` bound to the
     *         provided `domainSeparator`.
     *
