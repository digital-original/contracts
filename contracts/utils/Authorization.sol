 ...
 * @title Authorization
 *
 * @notice Mixin that provides EIP-712 based action authorization using a dedicated
 *         `SIGNER_ROLE` account. Intended to be inherited by contracts that need off-chain
 *         signatures for sensitive operations.
 *
 * @dev Relies on {EIP712Domain} for the domain separator and on {RoleSystem} for role
 *      storage. The helper {_requireAuthorizedAction} validates both signature freshness and
 *      signer authenticity.
 */
abstract contract Authorization is EIP712Domain, RoleSystem {
    /**
     * @notice Verifies that `signature` is a valid authorization for `messageHash` and that it
     *         has not expired.
     *
     * @dev Reverts with {AuthorizationDeadlineExpired} if `deadline` has passed or with
     *      {AuthorizationUnauthorizedAction} if the recovered signer does not hold the
     *      `SIGNER_ROLE`.
     *
