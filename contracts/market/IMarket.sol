...
interface IMarket {
    ...
    error MarketInvalidOrderSignature();

    /// @dev Thrown when an order is executed with an invalid side.
    error MarketInvalidOrderSide();

    /// @dev Thrown when the hash of the `order` does not match the `orderHash` in the `permit`.
    error MarketOrderHashMismatch();
